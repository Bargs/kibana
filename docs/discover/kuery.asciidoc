...
You may also use the range operators inside a grouped field query. This simplifies bounded ranges.

`response:(>=200 and <300)` will match any response greater than or equal to 200, and less than 300.

`bytes:(<1000 or >5000)` will match any doc with bytes less than 1000 or greater than 5000.
